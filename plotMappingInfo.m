function plotMappingInfo
%% Function written by Eleni Christoforidou in MATLAB R2019b.

%This function creates a plot of the Number of mapped reads vs the Type of 
%alignment.
...
stackData=zeros(3,6,3); %initialise variable.
for kk=1:d
    temp=series{kk};
    ...
    stackData(3,kk,1)=temp(2,2); %obtain number of unmapped alignments.
...
b=plotBarStackGroups(stackData,groupLabels); %create plot.
ylabel('Number of alignments','FontSize',12); %add y-axis label.
...
%Save figure.
savefig(gcf,'MappingInfo'); %save figure as a FIG file in the working directory.
fig.Renderer='painters'; %force MATLAB to render the image as a vector.
saveas(gcf,'MappingInfo.svg'); %save figure as an SVG file.
close
clear
end
