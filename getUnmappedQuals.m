...
%unmap_quals:       An M x 1 matrix of numbers, where M is equal to the
...
for f=1:d %loop through each TXT file.
    ...
    unmap_quals=zeros(length(seqs),1); %initialise variable.
    totalreads=length(seqs); %total number of unmapped reads.
    for ii=1:totalreads %loop through each unamapped read.
        unmap_quals(ii)=length(seqs{ii}); %calculate length of current sequence and save it in array of lengths.
    end
    ...
    save('qual_analysis.mat','unmap_quals'); %save the variable.
